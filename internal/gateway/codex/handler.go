...
func (h *Handler) RegisterRoutes(r *gin.Engine, authMiddleware gin.HandlerFunc, concurrencyLimiter *concurrency.Limiter, clientFilter gin.HandlerFunc) {
	...
	{
		v1.POST("/completions", middleware.ConcurrencyLimitMiddleware(concurrencyLimiter), middleware.AccessLogMiddleware(h.usageService), h.HandleCompletions)
		v1.POST("/engines/:engine/completions", middleware.ConcurrencyLimitMiddleware(concurrencyLimiter), middleware.AccessLogMiddleware(h.usageService), h.HandleCompletions)
	}
}
...
func (p *Protocol) FormatResponse(backendResp []byte) ([]byte, int, int, error) {
	...
	clientResp, err := ConvertFromOpenAI(backendResp, p.ClientReq, p.ModelConfig)
...
func (p *Protocol) FormatStreamLine(line string, state map[string]interface{}) (string, int, int, string, error) {
	clientLine, err := ConvertStreamLine(line, p.ClientReq, state, p.ModelConfig)
	if err != nil {
		return "", 0, 0, "", err
	}
